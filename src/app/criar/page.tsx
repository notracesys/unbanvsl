...
export default function CriarPagina() {
  ...
  const handleBack = () => {
    ...
    else if (phase === 'wizard') setPhase('dados');
    ...
    <div className="min-h-screen flex flex-col bg-background selection:bg-primary/30">
      ...
      <main className="flex-1 container max-w-lg mx-auto px-4 py-4 flex flex-col gap-6">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  {[1,2,3,4,5,6].map(s => (
                    <div key={s} className={cn("w-1 h-1 rounded-full", s === wizardStep ? "bg-primary w-2" : "bg-white/20")} />
                  ))}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 gap-3 pt-4">
          ...
          >
            {wizardStep === 6 ? "Concluir" : "Próximo Passo"} <ArrowRight className="ml-2 w-5 h-5" />
          </Button>
        </div>
      </main>
