...
export default function AdvancedAnalyticsDashboard() {
  ...
  // Busca simplificada para evitar erros de índice ausente no início
  ...
  const stats = useMemo(() => {
    ...
    // Ordena manualmente por data para garantir ordem cronológica no feed
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (dropPct > 5) { // Só mostra quedas relevantes
        dropoffs.push({
          range: `${i * 10}% a ${(i + 1) * 10}%`,
    ...
    return {
      totalSessions,
      avgWatchTimeStr: `${Math.floor(avgWatchSeconds / 60)}m ${avgWatchSeconds % 60}s`,
      avgCompletionPct,
      ctaClicks,
      conversionRate,
      retentionData,
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center bg-zinc-900/40 p-8 rounded-3xl border border-zinc-800">
          ...
          <h2 className="text-xl font-bold uppercase text-white mb-2">Sem Conexão</h2>
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <h2 className="text-white font-bold">Erro ao buscar dados</h2>
          <p className="text-zinc-500 text-sm mt-2">{error.message}</p>
          <Button onClick={() => setManualReload(s => s + 1)} className="mt-4 bg-zinc-800">Tentar Novamente</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 text-center">
        <div className="max-w-md space-y-6 bg-zinc-900/30 p-10 rounded-3xl border border-zinc-800/80 backdrop-blur-sm">
          ...
          <p className="text-zinc-400 text-sm leading-relaxed">
            Abra a sua página inicial em uma nova aba, dê o play no vídeo e assista um pouco. Os dados aparecerão aqui instantaneamente.
          </p>
          <Button 
          ...
          >
            <RefreshCw className="w-4 h-4 mr-2" /> Atualizar Dashboard
          </Button>
        </div>
      </div>
    );
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-zinc-900 pb-8">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-red-600 mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em]">Live Analytics Active</span>
            </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="flex items-center gap-3 bg-zinc-900/40 border border-zinc-800/60 px-4 py-2 rounded-xl">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">ID: {firebaseConfig.projectId}</span>
            </div>
          </div>
        </header>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard title="Sessões Totais" value={stats.totalSessions} icon={<Play className="text-red-600" />} subtitle="Visitantes monitorados" />
          <KpiCard title="Retenção Média" value={`${stats.avgCompletionPct}%`} icon={<Timer className="text-red-600" />} subtitle={stats.avgWatchTimeStr} />
          <KpiCard title="Cliques Checkout" value={stats.ctaClicks} icon={<MousePointerClick className="text-red-600" />} subtitle="Intenções de compra" />
          <KpiCard title="Conversão VSL" value={`${stats.conversionRate}%`} icon={<TrendingUp className="text-green-500" />} highlight />
        </div>
          ...
          <div className="space-y-6">
            <Card className="bg-zinc-900/20 border-zinc-800 backdrop-blur-sm">
              ...
              <CardContent className="space-y-3">
                {stats.dropoffs.length > 0 ? stats.dropoffs.map((d, index) => (
                ...
                )) : <p className="text-xs text-zinc-600 text-center py-4">Sem quedas bruscas detectadas.</p>}
              </CardContent>
            </Card>

            <Card className="bg-zinc-900/20 border-zinc-800 backdrop-blur-sm">
              <CardHeader className="pb-0">
                <CardTitle className="text-sm font-bold text-zinc-400 uppercase tracking-widest">Dispositivos</CardTitle>
              </CardHeader>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-zinc-900 bg-zinc-950/10">
                {stats.sortedMetrics.slice(0, 10).map((m: any, i: number) => (
                  <tr key={i} className="hover:bg-zinc-900/30 transition-colors">
                    <td className="p-4 font-mono text-zinc-400 text-[11px]">{m.visitorId?.substring(0, 8)}...</td>
                    <td className="p-4 uppercase text-[10px] font-bold text-zinc-400">{m.device || 'mobile'}</td>
                    <td className="p-4 font-bold text-white">{m.watchTime || 0}s</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right">
                      {m.clickedCTA ? (
                        <span className="px-2 py-0.5 rounded-md bg-green-950 text-green-400 font-black border border-green-800 text-[9px] uppercase tracking-wider">
                          Checkout
                        </span>
                      ) : (
